...
/*** List and struct to hold the waiting threads ***/
static struct list waitingList;
struct timedThread {
    struct thread *t;
...
timer_init (void)
{
  ...
  list_init(&waitingList);                              /// Initialize the list
}
...
static bool
waketime_less (const struct list_elem *_a,
...
timer_interrupt (struct intr_frame *args UNUSED)
{
  ticks++;
  thread_tick ();
  /// check for the waiting threads if it's time for one to wake up
  wakeup();

}

/*** wakes up the threads that are waiting(sleeping/blocked) */
void
wakeup(void) {
    struct list_elem *e;
        /// loop thru all elements in waiting List, this seem to be too much,
